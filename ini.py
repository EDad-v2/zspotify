...
USE_VBR = (
    True  # Encodes mp3 with variable bitrate, for smaller sizes. Does not affect ogg
)
FORCE_PREMIUM = False  # set to True if not detecting your premium account automatically
RAW_AUDIO_AS_IS = (
    False  # set to False if you wish you save the raw audio without re-encoding it.
)
...
class Zcfg:
    ...
    def get_token(cls):
        return (
            cls.SESSION.tokens()
            .get_token("user-read-email", "playlist-read-private", "user-library-read")
            .access_token
        )
    ...
    def check_premium(cls):
        return bool(
            (cls.SESSION.get_user_attribute("type") == "premium")
            or cls.get_force_premium()
        )
